...
func main() {
	...
	// S16 改动：hooks 继承 S15，不新增 hook 类型。
	loopinit.InitS16Hooks(hookBus, checker, workdir)
	...
	// S16 改动：常规 task subagent 仍沿用 S15 工具箱。
	subToolbox := loopinit.InitS16SubToolbox()
	...
	// S16 改动：Spawner 仍对标 Python active_teammates + spawn_teammate_thread。
	//
	// teammate 工具箱新增 submit_plan；persistent loop 由 S16 的 spawn 工具选择。
	spawner := team.NewSpawner(
		client,
		modelID,
		messageBus,
		func(agentName string) *v2.ToolBox {
			return loopinit.InitS16TeammateToolbox(messageBus, protocolBook, agentName)
		},
	)

	// S16 改动：Lead 工具箱切换为 S16，新增 request_shutdown/request_plan/review_plan。
	toolbox := loopinit.InitS16Toolbox(
		subAgent,
		skillRegistry,
		taskBoard,
		cronScheduler,
	...
	// S16 新增：system prompt 使用 V2 section 列表。
	//
	// V2 不再把课程新增片段写死在 prompt 包全局 map 中，而是由 S16 显式选择自己的 sections。
	promptSections := prompt.S16SectionsV2()
	...
	fmt.Println("s16: team protocols")
