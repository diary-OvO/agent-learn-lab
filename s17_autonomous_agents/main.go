...
func main() {
	...
	// S17 改动：hooks 继承 S16，不新增 hook 类型。
	loopinit.InitS17Hooks(hookBus, checker, workdir)
	...
	// S17 改动：常规 task subagent 仍沿用 S16 工具箱。
	subToolbox := loopinit.InitS17SubToolbox()
	...
	// S17 改动：Spawner 使用 autonomous 版本。
	//
	// autonomous teammate 在 IDLE 阶段会 scan_unclaimed_tasks 并自动 claim，因此这里额外注入 taskBoard。
	spawner := team.NewAutonomousSpawner(
		client,
		modelID,
		messageBus,
		taskBoard,
		func(agentName string) *v2.ToolBox {
			return loopinit.InitS17TeammateToolbox(messageBus, protocolBook, taskBoard, agentName)
		},
	)

	// S17 改动：Lead 工具箱继承 S16，spawn_teammate 切到 autonomous。
	toolbox := loopinit.InitS17Toolbox(
		subAgent,
		skillRegistry,
		taskBoard,
		cronScheduler,
	...
	// S17 继承 S16：system prompt 使用 V2 section 列表。
	//
	// S17 在 team_protocol 后追加 autonomous_team section。
	promptSections := prompt.S17SectionsV2()
	...
	fmt.Println("s17: autonomous agents")
