...
func ToolResultOutputHook() hooks.PostToolUseHook {
	return func(ctx context.Context, call v2.ToolCall, output string) string {
		formatted := strings.TrimSpace(agentconsole.FormatToolResult(ctx, call, output))
		if formatted != "" {
...
// 返回非空字符串表示阻止本次工具调用。
func PermissionHook(checker *permission.PermissionChecker) hooks.PreToolUseHook {
	return func(ctx context.Context, call v2.ToolCall) string {
		if checker == nil {
			return ""
		}

		if !checker.CheckPermission(ctx, call) {
