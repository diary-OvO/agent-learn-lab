...
func New(
	...
	if toolbox == nil {
		return nil, fmt.Errorf("subagent toolbox is nil")
	}
	toolboxSchema, err := openaiadapter.ToChatCompletionToolsV2(toolbox.Schemas())
	if err != nil {
		return nil, err
	}
	return &SubAgent{
...
func (a *SubAgent) Run(ctx context.Context, description string) (string, error) {
	...
	parentScope := agentconsole.ScopeFromContext(ctx)

	subCtx := agentconsole.WithAgentScope(ctx, agentconsole.AgentScope{
		Name:  "sub",
		ID:    id,
