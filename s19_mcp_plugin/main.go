...
func main() {
	...
	// S19 新增：MCP Registry 对标 Python mcp_clients + MOCK_SERVERS。
	//
	// 只保存教学版 mock MCP server 的连接状态；动态工具池仍在 main.go 每轮显式组装。
	mcpRegistry := mcp.NewRegistry()

	hookBus := hooks.NewHookBus()
	// S19 改动：hooks 继承 S18，不新增 hook 类型；MCP 工具仍走统一 hook。
	loopinit.InitS19Hooks(hookBus, checker, workdir)
	...
	// S19 改动：常规 task subagent 仍沿用 S18 工具箱，不接入 Lead 的 MCP 动态工具。
	subToolbox := loopinit.InitS19SubToolbox()
	...
	spawner := team.NewWorktreeAutonomousSpawner(
		...
		func(
			...
			return loopinit.InitS19TeammateToolbox(
				messageBus,
				protocolBook,
				taskBoard,
				agentName,
	...
	// S19 改动：Lead 固定工具箱继承 S18，并新增 connect_mcp。
	//
	// connect_mcp 发现的 mcp__{server}__{tool} 不放进固定 ToolBox，
	// 而是在 assembleOpenAIToolPool 中按当前 mcpRegistry 状态动态追加。
	toolbox := loopinit.InitS19Toolbox(
		subAgent,
		skillRegistry,
		taskBoard,
		cronScheduler,
	...
	// S19 新增：启动时先组装一次动态 tool pool，用于初始 prompt。
	//
	// 后续每轮请求前都会重新 assemble，保证 connect_mcp 后的新工具能进入模型请求。
	_, enabledTools, err := assembleOpenAIToolPool(toolbox, mcpRegistry)
	if err != nil {
		panic(err)
	}

	// S19 继承 S18：system prompt 使用 V2 section 列表。
	//
	// S19 在 worktree_isolation 后追加 mcp_tools section。
	promptSections := prompt.S19SectionsV2()
...
// assembleOpenAIToolPool 对标 Python S19 assemble_tool_pool。
//
// 迭代原因：S18 的工具列表在启动时固定；S19 执行 connect_mcp 后，需要下一轮模型请求
// 立刻看到 mcp__{server}__{tool} 动态工具。
//
// 与 S18 差别：这里每次合并 builtin toolbox、已连接 MCP tools 和 compact 控制工具，
// 但不把 MCP 动态工具注册回固定 ToolBox，避免旧章节行为被改写。
func assembleOpenAIToolPool(
	toolbox *v2.ToolBox,
	mcpRegistry *mcp.Registry,
) ([]openai.ChatCompletionToolUnionParam, []string, error) {
	schemas := toolbox.Schemas()

	if mcpRegistry != nil {
		schemas = append(schemas, mcpRegistry.ToolSchemas()...)
	}

	schemas = append(schemas, compact.CompactToolSchema())
	...
	return chatTools, v2.SchemaNames(schemas), nil
}

// runAgentTurn 对标 Python agent_loop 被 user/wake 事件调用的一次 Lead turn。
//
// S19 在 S18 基础上显式传入 MCP Registry，便于每个 turn 用真实连接状态重建动态 tool pool。
func runAgentTurn(
	ctx context.Context,
...
// runAgentLoop 对标 Python S19 agent_loop。
//
// 迭代原因：S19 的 tool pool 不再是启动时固定值，而要在每轮模型请求前合并
// builtin tools、已连接 MCP tools 和 compact 工具。
//
// 与 S18 差别：主 loop 骨架保持可见；只有请求前的工具池和工具执行分发变成 MCP-aware。
func runAgentLoop(
	ctx context.Context,
	client openai.Client,
	toolbox *v2.ToolBox,
	...
	for step := 0; step < maxSteps; step++ {
		...
		// S19 新增：每轮请求前动态重建 tool pool。
		//
		// 对标 Python agent_loop 中 assemble_tool_pool；connect_mcp 执行后，
		// 这里会把新发现的 MCP tools 追加进下一次模型请求。
		toolboxSchema, enabledTools, err := assembleOpenAIToolPool(toolbox, mcpRegistry)
		if err != nil {
			return "", messages, err
		}
		params.Tools = toolboxSchema
		...
		for _, toolCall := range msg.ToolCalls {
			...
			if bgTracker != nil && background.ShouldRun(toolCall.Function.Name, call.Arguments) {
				...
				task := bgTracker.Start(
					...
					call.Arguments,
					func() string {
						return executeToolCall(ctx, toolbox, mcpRegistry, hookBus, bgCall)
					},
				)
			...
			result := executeToolCall(ctx, toolbox, mcpRegistry, hookBus, call)
...
// executeToolCall 对标 Python S19 TOOL_HANDLERS[prefixed]。
//
// 迭代原因：S18 只从固定 ToolBox 执行工具；S19 连接 MCP 后会出现
// mcp__{server}__{tool} 动态工具，需要在执行时按前缀转交给 MCP Registry。
//
// 与 S18 差别：非 MCP 工具仍走固定 ToolBox；MCP 工具不注册进 ToolBox，而是动态分发。
func executeToolCall(
	...
	if mcpRegistry != nil && mcpRegistry.IsMCPTool(call.Name) {
		result, err = mcpRegistry.Execute(ctx, call)
	} else {
